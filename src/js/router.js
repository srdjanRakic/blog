(function() {
    ...
    var srdjanNavTitle = document.getElementById('srdjan-name');

    if (window.location.pathname !== "/") {
        nav.style.backgroundColor = "#000";
        nav.style.removeProperty('transition');
        srdjanNavTitle.style.removeProperty('transition');
        srdjanNavTitle.style.opacity = "1";
    ...
    window.addEventListener('scroll',function(evt) {
       if (window.pageYOffset >= 400 && window.location.pathname === '/') {
           ...
           nav.style.backgroundColor = "#000";
           srdjanNavTitle.style.transition = "opacity 0.4s linear";
           srdjanNavTitle.style.opacity = "1";
       } else {
           if (window.location.pathname === '/') {
               nav.style.backgroundColor = "transparent";
               srdjanNavTitle.style.opacity = "0";
           }
       }
    });
